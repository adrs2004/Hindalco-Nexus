...
export default function TeamManagementPage() {
  ...
  const fetchTeams = async () => {
    try {
      ...
      const { data } = await axios.get('https://hindalcobackend.onrender.com/api/teams', {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const adminInfo = JSON.parse(localStorage.getItem('adminInfo'));
      await axios.post('https://hindalcobackend.onrender.com/api/teams', formData, {
        headers: {
          Authorization: `Bearer ${adminInfo.token}`
        }
      });
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this team member?')) {
      try {
        const adminInfo = JSON.parse(localStorage.getItem('adminInfo'));
        await axios.delete(`https://hindalcobackend.onrender.com/api/teams/${id}`, {
          headers: {
            Authorization: `Bearer ${adminInfo.token}`,
          },
        });
